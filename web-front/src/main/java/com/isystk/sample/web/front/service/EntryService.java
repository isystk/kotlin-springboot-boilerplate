...
public class EntryService extends BaseTransactionalService {
  ...
  public void registTemporary(TUser tUser) {
    ...
    // 仮会員登録メールを送信する
    MMailTemplate mailTemplate = getMailTemplate(MailTemplate.ENTRY_REGIST_TEMPORARY.getCode());
  ...
  public void registComplete(String onetimeKey) {
    ...
    // 本会員登録完了メールを送信する
    MMailTemplate mailTemplate = getMailTemplate(MailTemplate.ENTRY_REGIST_VALID.getCode());
    String subject = mailTemplate.getTitle();
    String templateBody = mailTemplate.getText();
    EntryRegistTemporary dto = new EntryRegistTemporary();
    dto.setFamilyName(tUser.getFamilyName());
  ...
  protected MMailTemplate getMailTemplate(Integer templateId) {
    ...
    MMailTemplate mailTemplate = mMailTemplateDao.findOne(criteria).orElseThrow(
