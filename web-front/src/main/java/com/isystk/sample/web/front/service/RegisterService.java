...
public class RegisterService extends BaseTransactionalService {
  ...
  public void sendMail(BigInteger userId) {
    ...
    // 仮会員登録メールを送信する
    MailTemplate mailTemplate = mailTemplateRepository.getMailTemplate(MailTemplateDiv.ENTRY_REGIST_TEMPORARY);
  ...
  public void registComplete(String onetimeKey) {
    ...
    // 本会員登録完了メールを送信する
    MailTemplate mailTemplate = mailTemplateRepository.getMailTemplate(MailTemplateDiv.ENTRY_REGIST_VALID);
    String subject = mailTemplate.getTitle();
    String templateBody = mailTemplate.getText();
    MailEntryRegistTemporary dto = new MailEntryRegistTemporary();
    dto.setUserName(user.getName());
