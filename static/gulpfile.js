...
gulp.task(
  "ts-reload",
  gulp.series("compile-ts:dev", function (callback) {
    $.browserSync.reload();
    callback();
  })
);
...
gulp.task("copy-images:dev", function () {
  ...
  return gulp
    .src([config.globs.images, "!src/images/.gitkeep"])
...
gulp.task(
  ...
  gulp.series(
    ...
    function () {
      ...
      gulp.watch(config.globs.sass, gulp.series("bundle-sass:dev"));
      gulp.watch(config.globs.ts, gulp.series("ts-reload"));
      gulp.watch(config.globs.images, gulp.series("copy-images:dev"));
